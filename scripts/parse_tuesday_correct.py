...
def parse_tuesday_schedule(pdf_filename):
    """Extract Poole games from Tuesday Night Mens schedule"""
    games = []
    
    pdf_path = f"../source_data/{pdf_filename}"
                    ...
                    for row_idx, row in enumerate(table):
                        ...
                        if time_found and current_date and date_row_teams:
                            ...
                            for sheet_num in range(1, 9):  # 8 sheets max
                                ...
                                # Find non-empty team names from the cells
                                date_cell = next((t for t in date_teams if t and t != 'None'), None)
                                time_cell = next((t for t in time_teams if t and t != 'None'), None)
                                
                                if not date_cell or not time_cell:
                                    continue
                                
                                # Split by newlines to handle multiple games in same cell
                                date_cell_teams = [t.strip() for t in date_cell.split('\n') if t.strip()]
                                time_cell_teams = [t.strip() for t in time_cell.split('\n') if t.strip()]
                                
                                # Pair up teams line by line
                                for team1, team2 in zip(date_cell_teams, time_cell_teams):
...
if __name__ == "__main__":
    print("Parsing Tuesday Night Mens schedules...\n")
    
    all_games = []
    
    # Parse 1st Round
    print("=== Parsing 1st Round ===")
    games_round1 = parse_tuesday_schedule("Tuesday Night Mens 2025 2026 1st Round 26 teams.pdf")
    all_games.extend(games_round1)
    print(f"\n1st Round games found: {len(games_round1)}")
    
    # Parse 2nd Round
    print("\n=== Parsing 2nd Round ===")
    games_round2 = parse_tuesday_schedule("Tuesday Night Mens 2025 2026 2nd Round 26 teams.pdf")
    all_games.extend(games_round2)
    print(f"\n2nd Round games found: {len(games_round2)}")
    
    print(f"\n\nTotal Tuesday games found: {len(all_games)}")
    
    # Sort by date (simple sort - assumes chronological order)
    # You may want to improve this with proper date parsing
    
    # Save to file
    with open('../temp_data/tuesday_games.json', 'w') as f:
        json.dump(all_games, f, indent=2)
    
    print("Saved to temp_data/tuesday_games.json")
